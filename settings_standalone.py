"""
设置面板 — 独立进程运行，通过 config.json 通信
双击启动 / 由 main.py subprocess.Popen 调起
"""
import sys, os
...
class SettingsPanel:
    ...
    def _apply(self):
        ...
        self.cfg = cfg
        # 提示用户重启生效
        self._show_toast("✅ 配置已保存，重启可视化器后生效")
    ...
    def _show_toast(self, msg):
        ...
        x = self.win.winfo_x() + 100
        y = self.win.winfo_y() + self.win.winfo_height() - 60
        toast.geometry(f"+{x}+{y}")
        lbl = ttk.Label(toast, text=msg, background="#333", foreground="white", padding=8)
        lbl.pack()
